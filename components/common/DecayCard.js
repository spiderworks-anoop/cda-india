  ...
  useEffect(() => {
    ...
    const render = () => {
      let targetX = lerp(
        ...
        map(cursor.current.x, 0, winsize.current.width, -50, 50),
        0.1
      );
      let targetY = lerp(
        imgValues.imgTransforms.y,
        map(cursor.current.y, 0, winsize.current.height, -50, 50),
        0.1
      );
      let targetRz = lerp(
      ...
      if (targetX > bound) targetX = bound + (targetX - bound) * 0.1;
      if (targetX < -bound) targetX = -bound + (targetX + bound) * 0.1;
      if (targetY > bound) targetY = bound + (targetY - bound) * 0.1;
      if (targetY < -bound) targetY = -bound + (targetY + bound) * 0.1;
      ...
      imgValues.displacementScale = lerp(
        ...
        map(cursorTravelledDistance, 0, 100, 0, 200),
        0.06
      );
      ...
      >
        ...
        <g>
          <image
            ...
            preserveAspectRatio="xMidYMid slice" className="w-[710px]"
          />
        </g>
      </svg>
